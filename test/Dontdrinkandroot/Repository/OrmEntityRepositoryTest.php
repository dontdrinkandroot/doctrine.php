...
class OrmEntityRepositoryTest extends DoctrineOrmTestCase
{
    ...
    public function testFindPaginatedBy()
    {
        ...
        $paginator = $generatedIdExampleEntityRepository->findPaginatedBy(1, 2);
        $this->assertCount(2, $paginator->getIterator());
        $this->assertEquals(3, $paginator->count());

        $paginator = $generatedIdExampleEntityRepository->findPaginatedBy(2, 2);
        $this->assertCount(1, $paginator->getIterator());
        $this->assertEquals(3, $paginator->count());

        $paginator = $generatedIdExampleEntityRepository->findPaginatedBy(1, 10, ['id' => 3]);
        $this->assertCount(1, $paginator->getIterator());
        $this->assertEquals(1, $paginator->count());

        $paginator = $generatedIdExampleEntityRepository->findPaginatedBy(1, 10, [], ['name' => 'DESC']);
        /** @var GeneratedIdExampleEntity[] $results */
        $results = $paginator->getIterator()->getArrayCopy();
        $this->assertCount(3, $results);
        $this->assertEquals(3, $paginator->count());

        $this->assertEquals('yxcv', $results[0]->getName());
        $this->assertEquals('qwer', $results[1]->getName());
        $this->assertEquals('asdf', $results[2]->getName());
    }
